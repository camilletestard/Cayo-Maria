...
for (iter in 9:num_iter){ #for all iterations
  ...
  for (gy in 1:length(groupyears)){ #For each group
    ...
    for (h in 1:length(isPost)){ #pre- and post-hurricane 
      ...
      # Set PRE-HURRICANE standard Groom Strength CATEGORY as vertex attribute
      groomCat = as.character(SocialCapital$groomCat[match(as.character(net %v% "vertex.names"), as.character(SocialCapital$id))])
      set.vertex.attribute(net,"groomCat", groomCat)
    ...
    t<- try (mod<-stergm(prePostNet,
                         # formation = ~edges+nodematch("groomCat",diff=TRUE),
                         # formation = ~edges+gwesp(decay=0.1, fixed=T)+mutual+
                         #   edgecov("kinship")+edgecov("prox")+
                         #   nodematch("sex",diff=TRUE)+absdiff("groom")+ absdiff("age")+absdiff("rank")+absdiff("numP"),
                         formation = ~edges+gwesp(decay=0.1, fixed=T)+mutual+
                           edgecov("kinship")+edgecov("prox")+
                           nodematch("sex",diff=TRUE)+nodematch("groomCat",diff=TRUE)+
                           absdiff("age")+absdiff("rank"),
                         dissolution = ~edges+edgecov("kinship")+edgecov("prox")+
                           ...
                           nodematch("sex",diff=TRUE)+nodematch("groomCat",diff=TRUE)+absdiff("age")+ absdiff("rank"),
    ...
    if(!("try-error" %in% class(t))){ #if don't converge
      ...
      #                   "form.homoph.groom","form.homoph.age","form.homoph.rank","form.homoph.numP")] <- unlist(coef(mod)["formation"])
      TERGMeffects[1, c("form.edge", "form.triangle.close","form.reciprocity","form.kinship","form.prox","form.homoph.sexF","form.homoph.sexM",
                        "form.homoph.greg","form.homoph.shy","form.homoph.age","form.homoph.rank","form.homoph.numP")] <- unlist(coef(mod)["formation"])
      ...
      TERGMeffects[1, c("diss.edge","diss.kinship","diss.prox","diss.homoph.sexF","diss.homoph.sexM","diss.homoph.greg","diss.homoph.shy",
                        "diss.homoph.age","diss.homoph.rank","diss.homoph.numP")] <- unlist(coef(mod)["dissolution"])
    ...
    #Save
    save(TERGMeffects.ALL, file ="C:/Users/Camille Testard/Documents/GitHub/Cayo-Maria/R.Data/TERGMeffects2.RData")
  }
}
