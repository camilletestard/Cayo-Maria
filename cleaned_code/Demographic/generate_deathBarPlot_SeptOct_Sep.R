...
#study.pop.alive = filter(Demographics,Demographics$Status < as.Date("2017-09-17"))#Find deaths after the hurricane

Deaths.postHurr =  filter(Demographics,Demographics$DOD >= as.Date("2017-09-17") & Demographics$DOD < as.Date("2018-03-01"))#Find deaths after the hurricane
...
years=c(2016,2016,2016,2016,2017,2017,2017,2017,2017,2017,2017,2017,2017,2017,2017,2017,
        2018,2018,2018,2018,2018,2018,2018,2018,2018,2018)
months=c(09,10,11,12,01,02,03,04,05,06,07,08,09,10,11,12,01,02,03,04,05,06,07,08,09,10)

#I'll consider death in october to be deaths related to the hurricane.
monthYears=paste(months,years,sep=".")
alive=c(); deaths=c();death.month.adults=c(); my=1
for (my in 1:length(monthYears)){
  ...
  deaths[my] = length(which(Demographics.temp$DOD >= as.Date(paste(years[my],months[my],"01",sep="-")) & Demographics.temp$DOD <= as.Date(paste(years[my],months[my],"28",sep="-"))))
  death.month.adults[my]= deaths[my]/alive[my]*100
}

death.df=data.frame(month=c("sept.16","oct.16","nov.16","dec.16","jan.17","fev.17","mar.17","avr.17","mai.17","jun.17","jul.17","aug.17",
                 "sep.17","oct.17","nov.17","dec.17","jan.18","fev.18","mar.18","avr.18","mai.18","jun.18","jul.18","aug.18","sep.18","oct.18"),
                 death.per.alive=death.month.adults, deaths = deaths, alive = alive)
death.df$month <- factor(death.df$month, levels = c("sept.16","oct.16","nov.16","dec.16","jan.17","fev.17","mar.17","avr.17","mai.17","jun.17","jul.17","aug.17",
                                                    "sep.17","oct.17","nov.17","dec.17","jan.18","fev.18","mar.18","avr.18","mai.18","jun.18","jul.18","aug.18","sep.18","oct.18"))
setwd("C:/Users/Camille Testard/Desktop/Desktop-Cayo-Maria/Results/Demography")

death.plot<-ggplot(data=death.df, aes(x=month, y=death.per.alive)) +
  ...
  scale_x_discrete(labels= c("","nov.","","jan.","","mar.","","may","","jul.","",
                             "sept.","","nov.","","jan.","","mar.","","may","","jul.","","sept.","","nov.")) +
...
ggsave('deathplot_v1.png')
ggsave('deathplot_v1.eps', death.plot)
