...
num_iter = 350 #number of iterations
  ...
  for (iter in 1:num_iter){ #for all iterations
    ...
    for (g in 1:length(group)){ #For each group
      ...
      for (y in 1:length(years)){ #For each year in that group
        ...
        for (h in 1:length(isPost)){ #pre- and post-hurricane 
          ...
          #IMPORTANT NOTE: i do NOT standardize weights per group and year, otherwise i cannot see whether strength has icnreased or not
    ...
    for (g in 1:length(group)){ #For each group
      ...
      for (y in 1:length(years)){ #For each year
        ...
        #Save all
        save(population.strength.All, ID.strength.stableP.All, ID.strength.newP.All, ID.strength.oldP.All, file ="C:/Users/Camille Testard/Documents/GitHub/Cayo-Maria/R.Data/Strength.Stable.New.Old.Partners_minObs.RData")
...
save(population.strength.All, ID.strength.stableP.All, ID.strength.newP.All, ID.strength.oldP.All, file ="C:/Users/Camille Testard/Documents/GitHub/Cayo-Maria/R.Data/Strength.StablePartners_minObs.RData")
