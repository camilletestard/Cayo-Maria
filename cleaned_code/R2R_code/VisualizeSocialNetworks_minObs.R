...
load("C:/Users/Camille Testard/Documents/GitHub/Cayo-Maria/R.Data/DataPerGroup_minObs.RData")
...
a=1; numscans_list=list(); graph.dens.pre=vector(); graph.dens.post=vector()
for (a in 1:1){
  ...
  for (g in 1:length(group)){ #For each group
    ...
    for (y in 1:length(years)){ #For each year in that group
      ...
      for (h in 1:length(isPost)){ #pre- and post-hurricane 
        ...
        # REMOVE INDIVIDUALS WITH TWO FEW SCANS (reviewer request)
        IDs_out = as.character(numscans$id[which(numscans$freq<40)])
        unqIDs = setdiff(unqIDs, IDs_out)
        numscans = numscans[-which(numscans$freq<40),]
        ...
        if (network_action == "groom"){setwd("C:/Users/Camille Testard/Desktop/Desktop-Cayo-Maria/Results/SocialNetworkGraph/GroomNetworks/Sampling_R2R/test_v2")
          ...
          } else {title = paste("Grooming Network ",group[g],years[y]," post-hurricane, mean (SD) scans: ", mean_numScans_list[[g]][y]," (",sd_numScans_list[[g]][y],"); density:", graph.dens.post[gy],sep="")}}

        # if (network_action == "groom"){setwd("C:/Users/Camille Testard/Desktop/Desktop-Cayo-Maria/Results/SocialNetworkGraph/GroomNetworks/Sampling_R2R/test_minObs")
        ...
        #   } else {title = paste("Grooming Network ",group[g],years[y]," post-hurricane, mean (SD) scans: ", mean_num_scans," (",sd_num_scans,"); density:", graph.dens.post[gy],sep="")}}
        # 
        if (network_action == "prox"){setwd("C:/Users/Camille Testard/Desktop/Desktop-Cayo-Maria/Results/SocialNetworkGraph/ProxNetworks")
          ...
          } else {title = paste("Proximity Network ",group[g],years[y]," post-hurricane, mean (SD) scans: ", mean_numScans_list[[g]][y]," (",sd_numScans_list[[g]][y],"); density:", graph.dens.post[gy],sep="")}}
