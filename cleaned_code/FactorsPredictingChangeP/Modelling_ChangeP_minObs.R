...
iter=sample(max(full.data$iter),1)
...
write.csv(Estimates,"PredictProx_minObs.V.csv")
...
write.csv(Estimates,"PredictProx_minObs.KK.csv")
...
write.csv(Estimates,"PredictGroom_minObs.V.csv")
...
write.csv(Estimates,"PredictGroom_minObs.KK.csv")
#end of script
...
ggsave("ChangeProbProx_minObs.tiff", dpProx)
...
ggsave("ChangeProbGroom_minObs.tiff", dpGroom)
...
full.data.plot=full.data[full.data$iter==sample(n_iter,1),]

#Distribution of change in grooming
ggplot(full.data.plot, aes(x=dpSocial))+
  ...
  mean(full.data.plot$dpSocial); sd(full.data.plot$dpSocial)
ggsave("ChangeProbGroom_minObs.tiff", dpGroom)
ggsave("ChangeProbGroom_minObs.eps", dpGroom)
...
ggsave("preGroom_minObs.tiff",preGroom)
ggsave("preGroom_minObs.eps",preGroom)
...
ggsave("preDeath_minObs.tiff",preDeath)
ggsave("preDeath_minObs.eps",preDeath)
...
ggsave("dpAcc.dpSoc_minObs.tiff",dAcc.dSoc)
ggsave("dpAcc.dpSoc_minObs.eps",dAcc.dSoc)
