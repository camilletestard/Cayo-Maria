...
num_iter = 350 #set number of iterations
...
for (a in 1:length(actions)){ #for all actions
  ...
  iter=1
  for (iter in 67:num_iter){ #for all iterations
    ...
    for (g in 1:length(group)){ #For each group
      ...
      for (y in 1:length(years)){ #For each year in that group
        ...
        for (h in 1:length(isPost)){ #pre- and post-hurricane 
          ...
          save(AllStats,file ="C:/Users/Camille Testard/Documents/GitHub/Cayo-Maria/R.Data/AllStats_minObs.RData")
