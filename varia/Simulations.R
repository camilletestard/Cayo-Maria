...
#3. Does increasing the number of observations lead to a higher p(groom)
n = 500
scans = sample(1:10, n, replace=TRUE)
isGrooming = rep(0, n)
isGrooming[which(scans>5)] = 1
true.P.Groom = length(which(isGrooming == 1))/length(isGrooming)

p.groom = rep(0, n)
for (i in 1:n) {
  subscans = isGrooming[sample(1:n,i, replace=FALSE)]
  p.groom[i] = length(which(subscans == 1))/i
}
plot(p.groom, xlab = "Number of observations")
segments(x0=0,y0=true.P.Groom,x1=500,y1=true.P.Groom,col="red",lwd=2, lty=2)
