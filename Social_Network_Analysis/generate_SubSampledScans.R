...
obs.table.ID = as.data.frame(table(droplevels(as.factor(SubScans$focalID))))
obs.table.group = as.data.frame(table(droplevels(as.factor(SubScans$group))))
obs.table.quarter= as.data.frame(table(droplevels(as.factor(SubScans$Q))))
obs.table.timeBlock=as.data.frame(table(droplevels(as.factor(SubScans$timeBlock))))
obs.table.isPost = as.data.frame(table(droplevels(as.factor(SubScans$isPost))))
obs.table.sex = as.data.frame(table(droplevels(as.factor(SubScans$sex))))
...
#Save subsampled scan for this iterations
name = iter
AllSubScans[[nameAll]] <- SubScans
}
end_time <- Sys.time()
end_time - start_time

save(AllSubScans,file="AllSubScans.RData")