...
load("Social_Network_Analysis/SocialCapital.dSocialRates8.RData")
...
preHurrDeaths = filter(Demographics, Demographics$DOD >= as.Date("2014-09-16") & Demographics$DOD < as.Date("2017-09-16"))#Find births after hurricane
preYears = 2014:2017
preDeaths=matrix(NA, ncol=12, nrow=length(preYears))
for (y in 1:length(preYears)){ #for all years
...
postYears = 2017:2020
postDeaths=matrix(NA, ncol=length(postMonths), nrow=length(postYears))
...
################################################
# Find birth and death status of focal IDs
################################################
data = calcRandomScans(allScans)
...
for (iter in 1:num_iter){
  ...
  changeSocialRates$percentrank = as.numeric(changeSocialRates$percentrank)
  ...
  predictDeath.prox <- glmer(isDead~ dpAcc + sex + age + percentrank + (1|id), data = changeSocialRates, family = binomial(link = "logit"))
