  ...
  for(ii in 1:nrow(a)){ #for all observations
    if(!is.na(a[ii,2])) { #if not NA
      if(a[ii,ncol(a)] == 1 ) { #if isSocial
        if(a[ii,ncol(partners)+2] == 1){ #if isSocialGive
          count1=count1+1; S1 <- data.frame(givingID = as.character(a[ii, 1]), receivingID = as.character(a[ii, 2]))}
        else if (a[ii,ncol(partners)+3] == 1) { #if isSocialGet
          count2=count2+1; S1 <- data.frame(givingID = as.character(a[ii, 2]), receivingID = as.character(a[ii, 1]))}
